...
public class SalesforceTemplate extends AbstractOAuth2ApiBinding implements Salesforce, SalesforceTest {
    ...
    protected MappingJackson2HttpMessageConverter getJson1MessageConverter() {
    ...
    public <T> List<T> readList(JsonNode jsonNode, Class<T> type) {
        try {
            ...
            final JsonParser parser = objectMapper.treeAsTokens(jsonNode);
            return (List<T>) objectMapper.readValue(parser, listType);
        } catch (IOException e) {
